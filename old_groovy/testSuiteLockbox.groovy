...
def jobName = 'Lockbox'
def switchesForGradle = readFileFromWorkspace('jobs/build/resources/serenityTests/gradleForLockboxTestSuite.txt')
...
job("${pipeline}-${jobName}") {
    description(readFileFromWorkspace('jobs/build/resources/serenityTests/LockboxDescription.txt'))
    configure { project ->
