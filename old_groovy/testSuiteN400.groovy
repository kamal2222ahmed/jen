...
def jobName = 'N400'
def switchesForGradle = readFileFromWorkspace('jobs/build/resources/serenityTests/gradleForN400TestSuite.txt')
def elis_pipelines = ["ELIS2-VerifyTest", "ELIS2-HF-TS", "ELIS2-TestSuite"]
elis_pipelines.each {pipeline ->
job("${pipeline}-${jobName}") {
    description(readFileFromWorkspace('jobs/build/resources/serenityTests/N400Description.txt'))
    configure { project ->
    ...
    publishers {
        ...
        groovyPostBuild {
            script(readFileFromWorkspace('jobs/build/resources/serenityTests/serenityAfterTestReport.txt'))
            sandbox(true)
        }
    }
    steps {
