...
def jobName = 'DACA'
def switchesForGradle = readFileFromWorkspace('jobs/build/resources/serenityTests/gradleForDACATestSuite.txt')
...
job("${pipeline}-${jobName}") {
    description(readFileFromWorkspace('jobs/build/resources/serenityTests/DACADescription.txt'))
    configure { project ->
