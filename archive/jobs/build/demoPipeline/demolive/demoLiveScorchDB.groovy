job('ELIS2-DEMO-Live-ScorchUpdateLoad') {
  description(readFileFromWorkspace('resources/demoPipeline/demolive/demoLiveDBscorchDescription.txt'))
  jdk('java-1.8.0_u102')
  ...
  steps {
      shell(readFileFromWorkspace('resources/demoPipeline/demolive/dbDEMOLIVEGradlePropeties_stopTomcat.sh'))
      gradle {
          description('Database Clean + Scorch + Update')
          gradleName('gradle-2.7')
          passAsProperties(true)
          switches(readFileFromWorkspace('resources/demoPipeline/demolive/demoLiveGradleDBscorchUpdate.txt'))
      ...
      gradle {
          ...
          gradleName('gradle-2.7')
          passAsProperties(true)
          switches(readFileFromWorkspace('resources/demoPipeline/demolive/demoLiveDataLoader.txt'))
          tasks(':Shared:DataLoader:clean :Shared:DataLoader:build :Shared:DataLoader:loadFTData')
          rootBuildScriptDir('Apps')
          useWrapper(false)
          useWorkspaceAsHome(true)
      }
      shell(readFileFromWorkspace('resources/demoPipeline/demolive/demoLivedbConnect_tomcatStart.sh'))
    }
  configure { project ->
