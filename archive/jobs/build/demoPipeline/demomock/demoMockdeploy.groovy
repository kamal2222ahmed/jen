job('ELIS2-DEMO-Mock-Deploy') {
  description(readFileFromWorkspace('resources/demoPipeline/demomock/demoMockDeployDescription.txt'))
  jdk('java-1.8.0_u102')
  ...
  steps {
      shell(readFileFromWorkspace('resources/demoPipeline/demomock/preDeployDEMOMOCK.sh'))
    ...
    publishers {
      extendedEmail {
        triggers {
          failure {
            content(readFileFromWorkspace('resources/demoPipeline/demomock/demoMOCKemailFailureContent.txt'))
            recipientList('')
            sendTo {
              developers()
              culprits()
              recipientList()
            subject('Demo Mock Deployment Failure Notification')
            }
          }
          success {
            content(readFileFromWorkspace('resources/demoPipeline/demomock/demoMOCKemailSuccessContent.txt'))
            ...
            sendTo {
              developers()
              recipientList()
            subject('Deployment of Master Build Version ${"BTAR_VERSION"} to the Demo Mock Server is complete')
             }
          }
        }
      }
  ...
  configure { project ->
    def slack = project / 'publishers' / 'jenkins.plugins.slack.SlackNotifier' {
      ...
      customMessage('#${ENV,var="BTAR_VERSION"}')
    }
  }
  wrappers {
    credentialsBinding {
