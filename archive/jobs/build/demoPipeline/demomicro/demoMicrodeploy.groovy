job('ELIS2-DEMO-Micro-Deploy') {
  description(readFileFromWorkspace('resources/demoPipeline/demomicro/demoMicroServicesDeployDescription.txt'))
  jdk('java-1.8.0_u102')
  label('master')
  authorization {
  ...
  blockOn (['ELIS2-DEMO-Mock-ScorchUpdateLoad', 'System-.*']) {
      blockLevel('GLOBAL')
      }
  logRotator {
    daysToKeep(30)
  ...
  parameters {
    stringParam("ELIS2ENV", "DEMOMICRO")
  ...
  steps {
      shell(readFileFromWorkspace('resources/demoPipeline/demomicro/preDeployDEMOMICRO.sh'))
    ...
    publishers {
      extendedEmail {
        triggers {
          failure {
            content(readFileFromWorkspace('resources/demoPipeline/demomicro/demoMICROemailFailureContent.txt'))
            recipientList('#ELIS2-CI@uscis.dhs.gov', '#ELIS2-Skynet@uscis.dhs.gov', 'myUSCIS-Tech-Team@uscis.dhs.gov')
            sendTo {
              developers()
              culprits()
              recipientList()
            subject('Demo MicroServices Deployment Failure Notification')
            }
          }
          success {
            content(readFileFromWorkspace('resources/demoPipeline/demomicro/demoMICROemailSuccessContent.txt'))
            ...
            sendTo {
              developers()
              recipientList()
            subject('Deployment of Master Build Version ${BTAR_VERSION} to the Demo MicroServices Server is complete')
             }
          }
        }
      }
