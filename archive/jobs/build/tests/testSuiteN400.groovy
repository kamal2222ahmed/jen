...
def jobName = 'N400'
def switchesForGradle = readFileFromWorkspace('resources/serenityTests/gradleForN400TestSuite.txt')
...
job("${pipeline}-${jobName}") {
    description(readFileFromWorkspace('resources/serenityTests/N400Description.txt'))
    authorization {
    ...
    publishers {
        ...
        groovyPostBuild {
            script(readFileFromWorkspace('resources/serenityTests/serenityAfterTestReport.txt'))
            sandbox(true)
        }
    }
    steps {
