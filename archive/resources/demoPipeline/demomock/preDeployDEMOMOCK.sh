...
if [ -z $ELIS2_VERSION ];
then
   echo "We are going to deploy the latest integration gate build for you to the Demo Mock Server!"
   LAST_DEPLOY_VERSION=`jenkinsutil/grepVersion/get_previous_demo.rb LOCKBOX`
else
   LAST_DEPLOY_VERSION=$ELIS2_VERSION
   echo "We are going to deploy ELIS2 Version: $ELIS2_VERSION for you to the Demo Mock Server"
fi

#LAST_DEPLOY_VERSION="6.1.136.1975"
...
cp ${WORKSPACE}/toggles/toggles-demomock.sql ${WORKSPACE}/toggles.sql
