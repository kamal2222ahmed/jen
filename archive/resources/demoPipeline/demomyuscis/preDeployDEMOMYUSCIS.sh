...
LAST_DEPLOY_VERSION=`jenkinsutil/grepVersion/get_previous_demo.rb MYUSCIS`

if [ -z $ELIS2_VERSION ];
then
   echo "We are going to deploy the latest integration gate build for you to the Demo MyUSCIS Server!"
   LAST_DEPLOY_VERSION=`jenkinsutil/grepVersion/get_previous_demo.rb MYUSCIS`
else
   LAST_DEPLOY_VERSION=$ELIS2_VERSION
   BTAR_VERSION=$ELIS2_VERSION
   echo "We are going to deploy ELIS2 Version: $ELIS2_VERSION for you to the Demo MyUSCIS Server"
fi
...
cp ${WORKSPACE}/toggles/toggles-demomyuscis.sql ${WORKSPACE}/toggles.sql
