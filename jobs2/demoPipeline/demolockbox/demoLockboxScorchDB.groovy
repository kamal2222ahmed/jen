job('ELIS2-DEMO-Mock-Lockbox-ScorchUpdateLoad') {
  description(readFileFromWorkspace('resources2/demoPipeline/demolockbox/demoLockboxDBscorchDescription.txt'))
  jdk('java-1.8.0_u102')
  ...
  blockOn (['ELIS2-DEMO-Mock-Lockbox-Deploy', 'System-.*']) {
      blockLevel('GLOBAL')
      }
  logRotator {
    daysToKeep(30)
  ...
  steps {
      shell(readFileFromWorkspace('resources2/demoPipeline/demolockbox/dbDEMOLOCKBOXGradlePropeties_stopTomcat.sh'))
      gradle {
          description('Database Clean + Scorch + Update')
          gradleName('gradle-2.7')
          passAsProperties(true)
          switches(readFileFromWorkspace('resources2/demoPipeline/demolockbox/demoLockboxGradleDBscorchUpdate.txt'))
      ...
      gradle {
          ...
          gradleName('gradle-2.7')
          passAsProperties(true)
          switches(readFileFromWorkspace('resources2/demoPipeline/demolockbox/demoLockboxDataLoader.txt'))
          tasks(':Shared:DataLoader:clean :Shared:DataLoader:build :Shared:DataLoader:loadFTData')
          rootBuildScriptDir('Apps')
          useWrapper(false)
          useWorkspaceAsHome(true)
      }
      shell(readFileFromWorkspace('resources2/demoPipeline/demolockbox/demoLockboxdbConnect_tomcatStart.sh'))
    }
  configure { project ->
