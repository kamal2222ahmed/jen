job('ELIS2-DEMO-Mock-Lockbox-Deploy') {
  description(readFileFromWorkspace('resources2/demoPipeline/demolockbox/demoLockboxDeployDescription.txt'))
  jdk('java-1.8.0_u102')
  ...
  blockOn (['ELIS2-DEMO-Mock-Lockbox-ScorchUpdateLoad', 'System-.*']) {
      blockLevel('GLOBAL')
      }
  logRotator {
    daysToKeep(30)
  ...
  parameters {
    stringParam("ELIS2ENV", "DEMOLOCKBOX")
    ...
    stringParam("DB_USER", "ELIS2DATADEMOLBDEVELOP")
  }
  multiscm {
    git {
      remote {
        name('origin')
        url('${DEMOLOCKBOX_REPO_SSH}')
        branch('*/master')
  ...
  steps {
      shell(readFileFromWorkspace('resources2/demoPipeline//demolockbox/preDeployDEMOLOCKBOX.sh'))
      ...
      gradle {
          ...
          tasks(':Database:update -x :Database:updateDatabaseDba')
          useWrapper(false)
          useWorkspaceAsHome(true)
      }
      shell(readFileFromWorkspace('resources2/demoPipeline/demo_sql_and_deploy.sh'))
   ...
   publishers {
      extendedEmail {
        triggers {
          failure {
            content(readFileFromWorkspace('resources2/demoPipeline/demolockbox/demoLOCKBOXemailFailureContent.txt'))
            recipientList('#ELIS2-CI@uscis.dhs.gov', '#ELIS2-Skynet@uscis.dhs.gov', 'myUSCIS-Tech-Team@uscis.dhs.gov')
            sendTo {
              developers()
              culprits()
              recipientList()
            subject('Demo Lockbox Deployment Failure Notification')
            }
          }
          success {
            content(readFileFromWorkspace('resources2/demoPipeline/demolockbox/demoLOCKBOXemailSuccessContent.txt'))
            ...
            sendTo {
              developers()
              recipientList()
            subject('Deployment of Master Build Version ${ELIS2_VERSION} to the Demo Lockbox Server is complete')
             }
          }
        }
      }
