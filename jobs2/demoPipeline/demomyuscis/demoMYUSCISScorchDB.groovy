job('ELIS2-DEMO-Mock-MyUSCIS-ScorchUpdateLoad') {
  description(readFileFromWorkspace('resources2/demoPipeline/demomyuscis/demoMYUSCISDBscorchDescription.txt'))
  jdk('java-1.8.0_u102')
  ...
  blockOn (['ELIS2-DEMO-Mock-MyUSCIS-Deploy', 'System-.*']) {
      blockLevel('GLOBAL')
      }
  logRotator {
    daysToKeep(30)
  ...
  steps {
      shell(readFileFromWorkspace('resources2/demoPipeline/demomyuscis/dbDEMOMYUSCISGradlePropeties_stopTomcat.sh'))
      gradle {
          description('Database Clean + Scorch + Update')
          gradleName('gradle-2.7')
          passAsProperties(true)
          switches(readFileFromWorkspace('resources2/demoPipeline/demomyuscis/demoMYUSCISGradleDBscorchUpdate.txt'))
      ...
      gradle {
          ...
          gradleName('gradle-2.7')
          passAsProperties(true)
          switches(readFileFromWorkspace('resources2/demoPipeline/demomyuscis/demoMYUSCISDataLoader.txt'))
          tasks(':Shared:DataLoader:clean :Shared:DataLoader:build :Shared:DataLoader:loadFTData')
          rootBuildScriptDir('Apps')
          useWrapper(false)
          useWorkspaceAsHome(true)
      }
      shell(readFileFromWorkspace('resources2/demoPipeline/demomyuscis/demoMYUSCISdbConnect_tomcatStart.sh'))
    }
  configure { project ->
