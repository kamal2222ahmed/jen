job('ELIS2-DEMO-SITH-Deploy') {
  description(readFileFromWorkspace('resources2/demoPipeline/demosith/demoSITHDeployDescription.txt'))
  jdk('java-1.8.0_u102')
  ...
  blockOn (['ELIS2-DEMO-SITH-ScorchUpdateLoad', 'System-.*']) {
      blockLevel('GLOBAL')
      }
  logRotator {
    daysToKeep(30)
  ...
  steps {
      shell(readFileFromWorkspace('resources2/demoPipeline/demosith/preDeployDEMOSITH.sh'))
      environmentVariables {
          propertiesFile('${WORKSPACE}/properties_from_deploy.properties')
      }
      shell('./deploy-prep.sh')
      gradle {
    ...
    publishers {
      extendedEmail {
        triggers {
          failure {
            content(readFileFromWorkspace('resources2/demoPipeline/demosith/demoSITHemailFailureContent.txt'))
            recipientList('ryan.p.tighe@uscis.dhs.gov, jeshurun.s.johnson@uscis.dhs.gov')
            sendTo {
              //developers()
              //culprits()
              recipientList()
            subject('Demo SITH Deployment Failure Notification')
            }
          }
          success {
            content(readFileFromWorkspace('resources2/demoPipeline/demosith/demoSITHemailSuccessContent.txt'))
            recipientList('ryan.p.tighe@uscis.dhs.gov, jeshurun.s.johnson@uscis.dhs.gov')
            sendTo {
              //developers()
              recipientList()
            subject('Deployment of Master Build Version ${ELIS2_VERSION} to the Demo SITH Server is complete')
             }
          }
        }
      }
