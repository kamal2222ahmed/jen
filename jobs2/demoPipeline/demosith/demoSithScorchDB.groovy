job('ELIS2-DEMO-SITH-ScorchUpdateLoad') {
  description(readFileFromWorkspace('resources2/demoPipeline/demosith/demoSITHDBscorchDescription.txt'))
  jdk('java-1.8.0_u102')
  label('master')
  authorization {
  ...
  blockOn (['ELIS2-DEMO-SITH-Deploy', 'System-.*']) {
      blockLevel('GLOBAL')
      }
  logRotator {
    daysToKeep(30)
  ...
  steps {
      shell(readFileFromWorkspace('resources2/demoPipeline/demosith/dbDEMOSITHGradlePropeties_stopTomcat.sh'))
      gradle {
          description('Database Clean + Scorch + Update')
          gradleName('gradle-2.7')
          passAsProperties(true)
          switches(readFileFromWorkspace('resources2/demoPipeline/demosith/demoSITHGradleDBscorchUpdate.txt'))
      ...
      gradle {
          ...
          gradleName('gradle-2.7')
          passAsProperties(true)
          switches(readFileFromWorkspace('resources2/demoPipeline/demosith/demoSITHDataLoader.txt'))
          tasks(':Shared:DataLoader:clean :Shared:DataLoader:build :Shared:DataLoader:loadFTData')
          rootBuildScriptDir('Apps')
          useWrapper(false)
          useWorkspaceAsHome(true)
      }
      shell(readFileFromWorkspace('resources2/demoPipeline/demosith/demoSITHdbConnect_tomcatStart.sh'))
    }
  configure { project ->
