job('ELIS2-DEMO-Live-ScorchUpdateLoad') {
  description(readFileFromWorkspace('resources2/demoPipeline/demolive/demoLiveDBscorchDescription.txt'))
  jdk('java-1.8.0_u102')
  ...
  blockOn (['ELIS2-DEMO-Live-Deploy', 'System-.*']) {
      blockLevel('GLOBAL')
      }
  logRotator {
    daysToKeep(30)
  ...
  steps {
      shell(readFileFromWorkspace('resources2/demoPipeline/demolive/dbDEMOLIVEGradlePropeties_stopTomcat.sh'))
      gradle {
          description('Database Clean + Scorch + Update')
          gradleName('gradle-2.7')
          passAsProperties(true)
          switches(readFileFromWorkspace('resources2/demoPipeline/demolive/demoLiveGradleDBscorchUpdate.txt'))
      ...
      gradle {
          ...
          gradleName('gradle-2.7')
          passAsProperties(true)
          switches(readFileFromWorkspace('resources2/demoPipeline/demolive/demoLiveDataLoader.txt'))
          tasks(':Shared:DataLoader:clean :Shared:DataLoader:build :Shared:DataLoader:loadFTData')
          rootBuildScriptDir('Apps')
          useWrapper(false)
          useWorkspaceAsHome(true)
      }
      shell(readFileFromWorkspace('resources2/demoPipeline/demolive/demoLivedbConnect_tomcatStart.sh'))
    }
  configure { project ->
