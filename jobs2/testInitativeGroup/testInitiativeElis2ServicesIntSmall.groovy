job('ELIS2-TestInitiative-Elis2Services-Integration-Small') {
  ...
  steps {
      ...
      gradle {
          ...
          passAsProperties(true)
          rootBuildScriptDir('Apps/')
          switches(readFileFromWorkspace('resources2/ciPipeline/buildGate/buildGateBuildTestSonarInt/gradle-build-test-sonar-testInitiative.txt'))
          tasks(':Backend:Elis2Services:intTest -PtestType=smallTests runGulpFullNoInstall :Database:clean :Database:build :Workspace:javakeystore:createJKS -x sonarqube -x buildRpm -x runGulpFull')
          useWrapper(false)
          useWorkspaceAsHome(true)
      }
  }
