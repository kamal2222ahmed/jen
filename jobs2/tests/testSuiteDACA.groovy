...
def jobName = 'DACA'
def switchesForGradle = readFileFromWorkspace('resources2/serenityTests/gradleForDACATestSuite.txt')
...
job("${pipeline}-${jobName}") {
    description(readFileFromWorkspace('resources2/serenityTests/DACADescription.txt'))
    authorization {
    ...
    steps {
      if (pipeline == "ELIS2-Team-TestSuite") {
        ...
        gradle {
            ...
            if (pipeline == "ELIS2-Team-TestSuite") {
                switches(readFileFromWorkspace('resources2/serenityTests/teamgradleForDACATestSuite.txt'))
            } else {
