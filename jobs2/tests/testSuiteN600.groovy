...
def jobName = 'N600'
def switchesForGradle = readFileFromWorkspace('resources2/serenityTests/gradleForN600.txt')
...
job("${pipeline}-${jobName}") {
    description(readFileFromWorkspace('resources2/serenityTests/N600Description.txt'))
    authorization {
    ...
    steps {
      if (pipeline == "ELIS2-Team-TestSuite") {
        ...
        gradle {
            ...
            if (pipeline == "ELIS2-Team-TestSuite") {
                switches(readFileFromWorkspace('resources2/serenityTests/teamgradleForN600TestSuite.txt'))
            } else {
