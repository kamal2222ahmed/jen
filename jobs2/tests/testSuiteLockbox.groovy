...
def jobName = 'Lockbox'
def switchesForGradle = readFileFromWorkspace('resources2/serenityTests/gradleForLockboxTestSuite.txt')
...
job("${pipeline}-${jobName}") {
    description(readFileFromWorkspace('resources2/serenityTests/LockboxDescription.txt'))
    authorization {
    ...
    steps {
      if (pipeline == "ELIS2-Team-TestSuite") {
        ...
        gradle {
            ...
            if (pipeline == "ELIS2-Team-TestSuite") {
                switches(readFileFromWorkspace('resources2/serenityTests/teamgradleForLockboxTestSuite.txt'))
            } else {
