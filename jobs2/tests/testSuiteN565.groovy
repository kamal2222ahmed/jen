...
def jobName = 'N565'
def switchesForGradle = readFileFromWorkspace('resources2/serenityTests/gradleForN565.txt')
...
job("${pipeline}-${jobName}") {
    description(readFileFromWorkspace('resources2/serenityTests/N565Description.txt'))
    authorization {
    ...
    steps {
      if (pipeline == "ELIS2-Team-TestSuite") {
        ...
        gradle {
            ...
            if (pipeline == "ELIS2-Team-TestSuite") {
                switches(readFileFromWorkspace('resources2/serenityTests/teamgradleForN565TestSuite.txt'))
            } else {
