...
def jobName = 'IV'
def switchesForGradle = readFileFromWorkspace('resources2/serenityTests/gradleForIVTestSuite.txt')
...
job("${pipeline}-${jobName}") {
    description(readFileFromWorkspace('resources2/serenityTests/IVDescription.txt'))
    authorization {
    ...
    steps {
      if (pipeline == "ELIS2-Team-TestSuite") {
        ...
        gradle {
            ...
            if (pipeline == "ELIS2-Team-TestSuite") {
                switches(readFileFromWorkspace('resources2/serenityTests/teamgradleForIVTestSuite.txt'))
            } else {
