...
def jobName = 'TPS'
def switchesForGradle = readFileFromWorkspace('resources2/serenityTests/gradleForTPS.txt')
...
job("${pipeline}-${jobName}") {
    description(readFileFromWorkspace('resources2/serenityTests/TPSDescription.txt'))
    authorization {
    ...
    steps {
      if (pipeline == "ELIS2-Team-TestSuite") {
        ...
        gradle {
            ...
            if (pipeline == "ELIS2-Team-TestSuite") {
                switches(readFileFromWorkspace('resources2/serenityTests/teamgradleForTPSTestSuite.txt'))
            } else {
