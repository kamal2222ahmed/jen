cp demomock/gradle.properties Apps/gradle.properties
...
cd ${WORKSPACE}
cp toggles/toggles-demomock.sql toggles.sql
#Stop Tomcat
knife ssh "chef_environment:DEMOMOCK AND name:el2-DEMOMOCK-app1" -C 1 -a ipaddress -x chef "sudo chef-client -o utilities::stop_tomcats"
knife ssh "chef_environment:DEMOMOCK AND role:activemq*" "sudo service activemq-instance-ELIS2-int stop; sudo service activemq-instance-ELIS2-ext stop; sudo rm -rf /opt/apache-activemq-5.12.0/data /data/active-mq-data/*/kahadb/* /data/active-mq-data/*/*.log /data/active-mq-data/*/*.pid /data/active-mq-data/*/*.pid.stop ;  sudo service activemq-instance-ELIS2-int start; sudo service activemq-instance-ELIS2-ext start" -a ipaddress -x chef
