...
ELIS_VERSION_ARTIFACT=`echo ${ELIS_VERSION} | tr '-' '.'`
STATUS_FILE=Database-Deployment-Type-${ELIS_VERSION_ARTIFACT}.txt
NEXUS_TARGET=http://${NEXUS}/repository/releases/gov/dhs/uscis/elis2/Database/${ELIS_VERSION_ARTIFACT}
...
if [ $DISRUPTIVE = true ]; then
  ...
  ssh -oStrictHostKeyChecking=no -tt ec2-user@10.103.136.234 "sudo -u apache /var/www/sync/releases_sync.sh ${ELIS_VERSION_ARTIFACT}"
  exit 1
else
