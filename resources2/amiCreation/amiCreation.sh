...
ssh -oStrictHostKeyChecking=no -tt ec2-user@10.103.136.38 -i /home/jenkins/ssh/id_rsa "sudo -u apache /var/www/sync/releases_sync.sh ${VERSION}"
...
NEXUS_TARGET=http://10.193.129.166:8081/repository/releases/gov/dhs/uscis/elis2/AMI
export http_proxy=
if [ "${FORCE}" != "" ]; then
...
AMI_FILE=ami-${RELEASE_CANDIDATE}_sv.txt
...
until [ $n -ge 8 ]
 do
  ((curl -v -u jenkins:${NexusAdminPassword} --upload-file ${AMI_FILE} ${NEXUS_TARGET}/${AMI_FILE} ; ssh -oStrictHostKeyChecking=no -tt ec2-user@10.103.136.38 "sudo -u apache /var/www/sync/releases_sync.sh ${VERSION}") && break)
  n=$[$n+1]
  sleep 15
done
