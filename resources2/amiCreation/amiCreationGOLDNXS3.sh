...
gem install popen4
/opt/chefdk/embedded/bin/ruby ${WORKSPACE}/cloudops/amimaker/amimaker.rb -c gold_base-ami_sv.yml -e ${ENVIRONMENT} -r "${RELEASE_CANDIDATE}" | sudo tee output.txt
...
AMI_FILE=GoldAMI-LATEST.txt
timestamp=`date +"%Y-%m-%d-%H-%M-%S"`
sudo echo ami_id: $AMI_ID > ${AMI_FILE}
echo "AMI_ID=$AMI_ID" >> ${WORKSPACE}/amiIdFile.txt

NEXUS_TARGET=http://10.193.129.166:8081/repository/releases/gov/dhs/uscis/elis2/GoldAMI
curl --request DELETE --write "%{http_code} %{url_effective}\\n" --user jenkins:${NexusAdminPassword}  --output /dev/null --silent ${NEXUS_TARGET}/LATEST/GoldAMI.txt

curl -v -u jenkins:${NexusAdminPassword} --upload-file ${AMI_FILE} ${NEXUS_TARGET}/LATEST/${AMI_FILE}

curl -v -u jenkins:${NexusAdminPassword} --upload-file ${AMI_FILE} ${NEXUS_TARGET}/${timestamp}/GoldAMI-${timestamp}.txt
