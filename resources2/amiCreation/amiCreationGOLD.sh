...
AMI_FILE=latest-ami-id_sv.txt
AMI_FILE2=`date +"+%Y-%m-%d-%H-%M-%S"`-ami-id_sv.txt
...
curl --request DELETE --write "%{http_code} %{url_effective}\\n" --user jenkins:${NexusAdminPassword}  --output /dev/null --silent ${NEXUS_TARGET}/latest-ami-id_sv.txt
